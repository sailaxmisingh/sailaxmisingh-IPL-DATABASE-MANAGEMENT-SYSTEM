...
import java.sql.*;
public class ownee extends JFrame {
	...
	private JTextField pnationality;
	private JTextField pexperience;
...
JButton btnNewButton_ = new JButton("CREATE");
btnNewButton_.addActionListener(new ActionListener() {
	public void actionPerformed(ActionEvent e) {
		try{  
	  		 String query="insert into player(playerid,pname,pnationality,Pexperience,teamid) values(?,?,?,?,?)";
	  		 PreparedStatement pst=con.prepareStatement(query);
	  		 pst.setString(1, playerid.getText() );
	  		 pst.setString(2, pname.getText() );
	  		 pst.setString(3, pnationality.getText() );
	  		 pst.setString(4, pexperience.getText() );
	  		pst.setString(5, teamid.getText() );
	  		ResultSet rs=pst.executeQuery();
	  		rs.close();
	  		pst.close();con.close();
	  		}catch(Exception e1){
...
btnNewButton.setFont(new Font("Times New Roman", Font.BOLD, 20));
btnNewButton.setBounds(175, 443, 144, 47);
contentPane.add(btnNewButton);
